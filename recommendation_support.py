...
def _features(item: dict) -> frozenset[int]:
    """Build sparse metadata features without opening/decoding the media file."""
    out: set[int] = set()
...
def install(server_module, smart_mode_module) -> None:
    """Install recommendation endpoints without taking ownership of playback."""
        ...
        class RecommendationHandler(BaseHandler):
            def do_GET(self):
                ...
                if media.suffix.lower() not in video_exts:
                    ...
                    return
                data = _isolated_thumb(media, 360)
                if not data:
            ...
            def do_POST(self):
                ...
                try:
                    ...
                    payload = {"ok": False, "items": [], "error": str(exc)}
